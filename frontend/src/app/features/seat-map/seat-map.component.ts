      ...
      >
        <div class="max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div class="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-start">
            <div class="flex items-center gap-3">
              <div class="w-12 h-12 rounded-xl bg-davivienda text-white flex items-center justify-center font-black text-base shadow-md shadow-red-200">
                {{ myLockedSeat()?.seatNumber }}
              </div>
              <div>
                <div class="flex items-center gap-2">
                  <span class="text-sm font-bold text-slate-900">Asiento {{ myLockedSeat()?.seatNumber }}</span>
                  <span class="text-[10px] bg-red-100 text-davivienda font-bold px-2 py-0.5 rounded-full uppercase">
                    {{ myLockedSeat()?.seatClass }}
                  </span>
                </div>
                <p class="text-xs font-semibold text-slate-500">
                  Total: <span class="text-slate-900 font-black text-sm">$ {{ myLockedSeat()?.price | number }} COP</span>
                </p>
              </div>
            </div>

            <!-- 5-min TTL Countdown indicator -->
            <div class="flex flex-col items-end sm:items-start pl-4 border-l border-slate-200">
              <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Tiempo Restante:</span>
              <span class="font-mono text-base font-black text-davivienda animate-pulse">
          ...
          <div class="flex items-center gap-3 w-full sm:w-auto">
            <button
              (click)="releaseSeat()"
              class="flex-1 sm:flex-none px-4 py-2.5 rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-100 text-xs font-bold transition-colors"
            >
              Liberar Asiento
            </button>
            <button
              (click)="proceedToCheckout()"
              class="flex-1 sm:flex-none px-6 py-2.5 rounded-xl bg-davivienda hover:bg-red-700 text-white text-xs font-bold shadow-md shadow-red-200 transition-all flex items-center justify-center gap-2"
            >
...
export class SeatMapComponent implements OnInit, OnDestroy {
  ...
  public isSeatMine(seat: Seat): boolean {
    ...
    return seat.status === SeatStatus.LOCKED && seat.lockedByUserId === currentUserId;
  }
  ...
  public onSeatClick(seat: Seat) {
    if (this.isSeatMine(seat)) {
      // Si hace click en su propio asiento, preguntar si desea liberarlo
      if (confirm(`¿Deseas liberar el asiento ${seat.seatNumber}?`)) {
